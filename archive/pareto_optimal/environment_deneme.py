...
class Env():
    ...
    def step(self, action_idx):
        ...
        if self.avt_value >= 25:
            reward_avt += (self.avt_value - self.previous_avt)  # AVT'deki artışı ödüllendir
        else:
            reward_avt -= 10  # AVT 25'in altındaysa ceza

        # JPH için ödül
        if self.jph_value > self.previous_jph:
            reward_jph += (self.jph_value - self.previous_jph)  # JPH'deki artışı ödüllendir
        else:
            reward_jph -= 5  # JPH azalırsa ceza

        reward_vector = [reward_avt, reward_jph]
