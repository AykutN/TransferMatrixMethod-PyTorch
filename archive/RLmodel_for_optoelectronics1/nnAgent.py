...
class ReplayBuffer():
    ...
    def store(self, state, action, reward, next_state, done, avt_value, highest_avt):
        ...
        self.highest_cri = highest_cri
        priority = reward
        if avt_value > self.highest_avt:
            priority += avt_value - self.highest_avt
        """
        if cri_value > self.highest_cri:
            priority += cri_value - self.highest_cri
        """
