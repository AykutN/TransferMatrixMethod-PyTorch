...
class MultiObjectiveQNetwork(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim=256):
        ...
        self.shared = nn.Sequential(
            ...
            nn.LayerNorm(hidden_dim),  # BatchNorm1d yerine LayerNorm
            nn.ReLU(),
...
class DQNAgent:
    ...
    def select_action(self, state):
        if np.random.rand() < self.epsilon:
            ...
            state = T.tensor(state, dtype=T.float32).reshape(1, -1).to(self.device)
            q_avt, q_jph = self.q_network(state)
            # AVT değerine göre dinamik ağırlıklandırma
            avt_weight = 0.3 if self.avt_value >= 25 else 0.7
            jph_weight = 1 - avt_weight
            q_values = avt_weight * q_avt + jph_weight * q_jph
    ...
    def train(self):
        ...
        avt_weight = 0.3 if self.avt_value >= 25 else 0.7  # 0.8 yerine 0.7
        jph_weight = 1 - avt_weight
        total_loss = avt_weight * loss_avt + jph_weight * loss_jph
