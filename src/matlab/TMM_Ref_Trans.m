function [reflectivity,transmissivity,angle_out] = TMM_Ref_Trans(wavelength, thicknesses,layers,angle)
...
Rp = zeros(length(wL),1);
Rs = zeros(length(wL),1);
Tp = zeros(length(wL),1);
Ts = zeros(length(wL),1);
...
rp = (N(w,L2)*cos(o(L1))- N(w,L1)*cos(o(L2))) / (N(w,L1)*cos(o(L2))+ N(w,L2)*cos(o(L1)));
rs = (N(w,L1)*cos(o(L1))- N(w,L2)*cos(o(L2))) / (N(w,L1)*cos(o(L1))+ N(w,L2)*cos(o(L2)));

Rp(w) = rp.*conj(rp);
Rs(w) = rs.*conj(rs);

tp = 2*N(w,L1).*cos(o(L1))./(N(w,L2).*cos(o(L1)) + N(w,L1).*cos(o(L2)));
ts = 2*N(w,L1).*cos(o(L1))./(N(w,L1).*cos(o(L1)) + N(w,L2).*cos(o(L2)));

Ts(w) = real(N(w,L2) .* cos(o(L2)))./real(N(w,L1).*cos(o(L1))).*ts.*conj(ts);
Tp(w) = real(N(w,L2) .* cos(o(L2)))./real(N(w,L1).*cos(o(L1))).*tp.*conj(tp);
else
...
Tp(w) = real(conj(N(w,n)) * cos(o(n))) / real(conj(N(w,1)) * cos(o(1))) * (t_p*conj(t_p));
...
Ts(w) = real(N(w,n) * cos(o(n))) / real(N(w,1) * cos(o(1))) * (t_s*conj(t_s));

end

end

reflectivity =  0.5.*(Rp + Rs);
transmissivity = 0.5.*(Tp + Ts);
